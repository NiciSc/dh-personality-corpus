"""This module validates if scrapted JSONS:
    ...
    and either deletes faulty JSONs or moves them"""
import csv
import datetime
import json
import os
...
for root, dirs, files in os.walk('.'):
    for file in files:
        if file.endswith(".json"):
            ...
            # Wenn ein Fehler beim Zugriff auf die Daten passiert,
            # wird das File in manualReview Ordner verschoben
            except:
                print(
                    rf"Fehler bei ID: {idFromFilename} - in manualReview Ordner verschoben")
                os.rename(rf'{idFromFilename}.json',
                          rf'{REVIEW_FOLDER}/No Access to Data - {idFromFilename}.json')
                reviewCount += 1
                data = []
                continue
            ...
            # Alle Profile ohne ID sehen so aus: https://imgur.com/a/2LJden3
                ...
                print(
                    rf'Datei {idFromFilename}.json hat keine ID\
                    - in manualReview Ordner verschoben')
                fileCount -= 1
                # Das fehlerhafte Profil wird in manualReview Ordner verschoben.
                # Es wird nicht in die .csv geschrieben, da es nicht nochmal gescraped werden soll
                os.rename(rf'{idFromFilename}.json',
                          rf'{REVIEW_FOLDER}/No ID in Data - {idFromFilename}.json')
                reviewCount += 1
    break
csvFile.close()
...
if reviewCount > 0:
    print(rf'{reviewCount} file(s) moved to manualReview folder\
    - please manually review these files.')
